...
public class OauthUtility {
    ...
    private static String buildURI(String host, String location,
            boolean isSecure) {
    ...
    public static KuaipanURL buildGetURL(String host, String location,
            Map<String, String> params, ConsumerToken consumer,
            TokenPair token, boolean isSecure) {
        KuaipanURL kpurl = buildURL("GET", host, location, params, consumer,
                token, isSecure);
        kpurl.convert2Get();
        return kpurl;
    }
    ...
    private static String generateSignature(String method, String host,
            String location, TreeMap<String, String> params,
            ConsumerToken consumer, TokenPair token, boolean isSecure)
            throws NoSuchAlgorithmException, InvalidKeyException {

        String msg = generateBaseString(method, host, location, params,
                consumer, token, isSecure);
    ...
    private static String generateSecret(ConsumerToken consumer, TokenPair token) {
        StringBuffer buf = new StringBuffer();
        buf.append(consumer.oauth_token_secret);
        buf.append(JOIN_AND);
        if (token != null)
            buf.append(token.oauth_token_secret);
     ...
     *            GET or POST.
     * @param host
     *            like 'openapi.kuaipan.cn', without scheme.
     * @param location
     *            starts with '/', no non-ASCII characters.
     * @param params
     *            without tokens.
     * @param consumer
     * @param token
     * @param isSecure
     *            whether uses HTTPS or not.
     * @return basestring defined in RFC 5849 3.4.1. Signature Base String
     */
    private static String generateBaseString(String method, String host,
            String location, TreeMap<String, String> params,
            ConsumerToken consumer, TokenPair token, boolean isSecure) {
    ...
    private static String normalizeParameters(TreeMap<String, String> params,
            ConsumerToken consumer, TokenPair token) {
        StringBuffer buf = new StringBuffer();
        TreeMap<String, String> tm = new TreeMap<String, String>(params);

        tm.put("oauth_consumer_key", consumer.oauth_token);
        if (token != null)
            tm.put("oauth_token", token.oauth_token);
    ...
    /**
     * unreserved characters (ALPHA / DIGIT / "-" / "." / "_" / "~") MUST not be
     * encoded, others MUST be encoded.
     * 
     * @param str
    ...
    private static String oauthEncode(String str) {
        try {
            return URLEncoder.encode(str, ENCODE).replace("*", "%2A").replace(
                    "%7E", "~").replace("+", "%20");
        } catch (UnsupportedEncodingException e) {
            return null;
        }
